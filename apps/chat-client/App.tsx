...
import { type ChatMessage, type CustomerAddress, type PaymentInstrument, type Product, Sender, type Checkout, type PaymentHandler } from './types';
...
function App() {
  ...
  const handleSelectAddress = (addressId: string) => {
    // Remove address selector from messages
    setMessages((prev) => prev.filter((msg) => !msg.customerAddresses));

    const userActionMessage = createChatMessage(
      Sender.USER,
      `Selected address: ${addressId}`,
      { isUserAction: true },
    );
    setMessages((prev) => [...prev, userActionMessage]);

    const actionPayload = JSON.stringify({
      action: 'select_customer_address',
  ...
  const handleSendMessage = async (
    ...
    try {
      ...
      const defaultHeaders = {
        ...
        'UCP-Agent': 'profile="https://business-agent-178484567461.us-central1.run.app/.well-known/ucp"',
      };
      ...
      for (const part of responseParts) {
        ...
        } else if (part.data?.['a2a.customer_addresses']) {
          // Customer addresses for registered user
          combinedBotMessage.customerAddresses =
            part.data['a2a.customer_addresses'];
